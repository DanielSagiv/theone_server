/**
 * Read-only: compare TAO Beach seat codes between stage and prod.
 *
 * Usage:
 *   STAGE_MONGODB_URI=... PROD_MONGODB_URI=... node scripts/verify-tao-beach-location-parity.js
 *
 * Optional:
 *   TAO_BEACH_LOCATION_ID=...
 */

require('dotenv').config();
const { MongoClient, ObjectId } = require('mongodb');
const { DEFAULT_LOCATION_ID } = require('../utils/taoBeachVenueConfig');

const LOG = '[verify-tao-beach-location-parity]';
...
async function main() {
  ...
  const locationId = process.env.TAO_BEACH_LOCATION_ID || DEFAULT_LOCATION_ID;
  ...
  try {
    ...
    console.log(`${LOG} Location: ${stageLoc.name || 'TAO Beach'} (${locationId})`);
