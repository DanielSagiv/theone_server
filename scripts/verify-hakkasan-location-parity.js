/**
 * Read-only: compare Hakkasan Las Vegas seat codes between stage and prod.
 *
 * Usage:
 *   STAGE_MONGODB_URI=... PROD_MONGODB_URI=... node scripts/verify-hakkasan-location-parity.js
 *
 * Optional:
 *   HAKKASAN_LOCATION_ID=...
 */

require('dotenv').config();
const { MongoClient, ObjectId } = require('mongodb');
const { DEFAULT_LOCATION_ID } = require('../utils/hakkasanVenueConfig');

const LOG = '[verify-hakkasan-location-parity]';

const EXPECTED_SEAT_CODES = [
  'Main Room Owners',
  'Main Room Stage',
  'Main Room Lower Dance Floor',
  'Main Room Upper Dancefloor',
  'Main Room 3rd/4th Rows',
  'Mezzanine Center',
  'Mezzanine Side',
  'Mezzanine Skybox',
].sort();
...
async function main() {
  ...
  const locationId = process.env.HAKKASAN_LOCATION_ID || DEFAULT_LOCATION_ID;
  ...
  try {
    ...
    console.log(`${LOG} Location: ${stageLoc.name || 'Hakkasan'} (${locationId})`);
