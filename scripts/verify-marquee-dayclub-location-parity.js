/**
 * Read-only: compare Marquee Dayclub seat codes between stage and prod.
 *
 * Usage:
 *   STAGE_MONGODB_URI=... PROD_MONGODB_URI=... node scripts/verify-marquee-dayclub-location-parity.js
 *
 * Optional:
 *   MARQUEE_DAYCLUB_LOCATION_ID=...
 */

require('dotenv').config();
const { MongoClient, ObjectId } = require('mongodb');
const { DEFAULT_LOCATION_ID } = require('../utils/marqueeDayclubVenueConfig');

const LOG = '[verify-marquee-dayclub-location-parity]';
...
async function main() {
  ...
  const locationId = process.env.MARQUEE_DAYCLUB_LOCATION_ID || DEFAULT_LOCATION_ID;
  ...
  try {
    ...
    console.log(`${LOG} Location: ${stageLoc.name || 'Marquee Dayclub'} (${locationId})`);
