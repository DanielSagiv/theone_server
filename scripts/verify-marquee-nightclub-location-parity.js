/**
 * Read-only: compare Marquee Nightclub seat codes between stage and prod.
 *
 * Usage:
 *   STAGE_MONGODB_URI=... PROD_MONGODB_URI=... node scripts/verify-marquee-nightclub-location-parity.js
 *
 * Optional:
 *   MARQUEE_NIGHTCLUB_LOCATION_ID=...
 */

require('dotenv').config();
const { MongoClient, ObjectId } = require('mongodb');
const { DEFAULT_LOCATION_ID } = require('../utils/marqueeNightclubVenueConfig');

const LOG = '[verify-marquee-nightclub-location-parity]';

const EXPECTED_SEAT_CODES = [
  'Cloud',
  'Dance Floor',
  'Full Upper Dance Floor',
  'Salon',
  'Third Tier Main Room',
  'Upper Dance Floor',
].sort();
...
async function main() {
  ...
  const locationId = process.env.MARQUEE_NIGHTCLUB_LOCATION_ID || DEFAULT_LOCATION_ID;
  ...
  try {
    ...
    console.log(`${LOG} Location: ${stageLoc.name || 'Marquee Nightclub'} (${locationId})`);
