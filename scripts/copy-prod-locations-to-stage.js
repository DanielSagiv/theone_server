/**
 * Copy ALL locations from production MongoDB into stage (upsert by _id).
 * Prod is read-only. Stage is the only write target. Nothing is deleted on either env.
 *
 * Usage (from server/):
 *
 *   # 1) Preview (no writes)
 *   node scripts/copy-prod-locations-to-stage.js --use-env-uris --dry-run
 *
 *   # 2) Apply (backs up affected stage docs first)
 *   node scripts/copy-prod-locations-to-stage.js --use-env-uris --confirm \
 *     --backup-dir ./backups/stage-locations-before-prod-sync-$(date +%Y%m%d)
 *
 ...
 *     node scripts/copy-prod-locations-to-stage.js --dry-run
 *
 * Safety:
...
const LOG = '[copy-prod-locations-to-stage]';
 ...
 * @returns {string}
 */
function docLabel(name, doc) {
  return doc.name || doc.title || doc.venue_name || doc.slug || name;
}

/**
 * Backup stage location docs that will be overwritten (matched prod _id).
 * @param {import('mongodb').Db} stageDb
 * @param {import('mongodb').ObjectId[]} prodIds
 * @param {string} backupDir
 */
async function backupStageLocations(stageDb, prodIds, backupDir) {
  const stageCol = stageDb.collection(COLLECTION_LOCATIONS);
  const existing = await stageCol.find({ _id: { $in: prodIds } }).toArray();
  fs.mkdirSync(backupDir, { recursive: true });
  const filePath = path.join(backupDir, 'stage-locations-before-prod-sync.json');
  fs.writeFileSync(
    filePath,
    JSON.stringify(
      {
  ...
  console.log(`${LOG} backed up ${existing.length} existing stage location(s) -> ${filePath}`);
  return filePath;
}

/**
 * Copy all prod locations to stage by _id upsert.
 * @param {import('mongodb').Db} prodDb
 * @param {import('mongodb').Db} stageDb
 * @param {boolean} dryRun
 * @returns {Promise<{ prodCount: number, inserted: number, replaced: number }>}
 */
async function copyAllLocations(prodDb, stageDb, dryRun) {
  const prodCol = prodDb.collection(COLLECTION_LOCATIONS);
  const stageCol = stageDb.collection(COLLECTION_LOCATIONS);

  const prodDocs = await prodCol.find({}).toArray();
  console.log(`${LOG} prod locations to copy: ${prodDocs.length}`);

  let inserted = 0;
  let replaced = 0;

  for (const doc of prodDocs) {
    const label = docLabel(doc._id.toString(), doc);
    if (dryRun) {
      const exists = await stageCol.findOne({ _id: doc._id }, { projection: { _id: 1, name: 1 } });
      console.log(
        `[dry-run] ${doc._id} "${label}" -> would ${exists ? 'replace' : 'insert'} on stage`,
      );
    ...
    const result = await stageCol.replaceOne({ _id: doc._id }, doc, { upsert: true });
    if (result.upsertedCount) inserted += 1;
    else if (result.modifiedCount || result.matchedCount) replaced += 1;
    console.log(`${LOG} ${doc._id} "${label}" -> stage (${result.upsertedCount ? 'inserted' : 'replaced'})`);
  }

  return { prodCount: prodDocs.length, inserted, replaced };
}
...
async function main() {
  ...
  if (!dryRun && !confirm) {
    console.error(
      `${LOG} Live run requires --confirm. Run --dry-run first to preview.`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const stageBefore = await stageDb.collection(COLLECTION_LOCATIONS).countDocuments();
    const prodCount = await prodDb.collection(COLLECTION_LOCATIONS).countDocuments();
    console.log(`${LOG} stage locations before: ${stageBefore}`);
    console.log(`${LOG} prod locations: ${prodCount}`);

    if (!dryRun) {
      const prodIds = await prodDb
      ...
      const dir =
        ...
        path.join(
          process.cwd(),
          'backups',
          `stage-locations-before-prod-sync-${new Date().toISOString().slice(0, 10)}`,
        );
      await backupStageLocations(stageDb, prodIds, dir);
    }

    const stats = await copyAllLocations(prodDb, stageDb, dryRun);

    const stageAfter = dryRun
      ? stageBefore
      : await stageDb.collection(COLLECTION_LOCATIONS).countDocuments();

    console.log('\nSummary');
    console.log(`prod locations copied: ${stats.prodCount}`);
    console.log(`would insert / inserted: ${stats.inserted}`);
    console.log(`would replace / replaced: ${stats.replaced}`);
    console.log(`stage locations after: ${dryRun ? `${stageBefore} (unchanged, dry-run)` : stageAfter}`);
