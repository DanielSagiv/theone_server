...
} = require('./hakkasanEventDetailScraperService');
const { isHakkasanStudioEvent } = require('./hakkasanScraperService');
const { resolveVenueTimezone, wallClockInVenueTzToUtcDate } = require('../../utils/venueTimezone');
const { getHakkasanVenueConfig, inferHakkasanFromEventCode } = require('../../utils/hakkasanVenueConfig');
const {
...
function parseHakkasanStartTimeTo24h(timeStr) {
...
function buildStartDatetimeLocal(isoDate, startTime) {
  ...
  const time24 = parseHakkasanStartTimeTo24h(startTime) || '22:00';
  return `${isoDate}T${time24}`;
}

function isHakkasanImportableRow(listingEvent) {
  if (!listingEvent || !listingEvent.eventCode || listingEvent.isCustomPromo) return false;
  if (listingEvent.isStudioEvent || isHakkasanStudioEvent(listingEvent.name)) return false;
  return inferHakkasanFromEventCode(listingEvent.eventCode);
}

function parseHakkasanDateQueryParam(value) {
  return parseScrapDateQueryParam(value);
}

function validateHakkasanDateRangeQuery(fromDate, toDate) {
  return validateScrapDateRangeQuery(fromDate, toDate, 'HAKKASAN_INVALID_DATE_RANGE');
}

function filterHakkasanEventsByDateRange(events, fromDate, toDate) {
  return filterScrapEventsByDateRange(events, fromDate, toDate);
}

function filterHakkasanEventsNotInPast(events) {
  return filterScrapEventsNotInPast(events);
}

function enrichHakkasanInventorySeatCodes(items) {
...
async function partitionHakkasanEventsByImportStatus(events) {
  const allRows = Array.isArray(events) ? events : [];
  const importable = [];
  const skipped = [];

  allRows.forEach((row) => {
    if (isHakkasanImportableRow(row)) {
      importable.push(row);
    } else {
      skipped.push(row);
    }
  ...
  const existingRows =
    codes.length > 0
      ? await Event.find({ hakkasanEventCode: { $in: codes } }).select('hakkasanEventCode name').lean()
      : [];
  const existingByCode = new Map(existingRows.map((e) => [e.hakkasanEventCode, e]));
...
function ensureHakkasanEventCodeOnDetailUrl(detailUrl, hakkasanEventCode) {
  const url = (detailUrl || '').trim();
  const code = (hakkasanEventCode || '').trim();
  if (!url) return null;
...
function extractHakkasanDetailUrlFromNotes(notes) {
  if (!notes) return null;
  const match = String(notes).match(/Source:\s*(https?:\/\/\S+)/i);
  return match ? match[1].trim() : null;
}

function resolveHakkasanDetailUrl(notes, hakkasanEventCode) {
  const fromNotes = extractHakkasanDetailUrlFromNotes(notes);
...
async function prepareHakkasanEventImport(listingEvent, options = {}) {
  ...
  if (!hakkasanEventCode) {
    const err = new Error('Missing Hakkasan event code');
    err.code = 'HAKKASAN_MISSING_EVENT_CODE';
    throw err;
  }

  if (!isHakkasanImportableRow(listingEvent)) {
    const err = new Error(
      listingEvent.isStudioEvent || isHakkasanStudioEvent(listingEvent.name)
        ? 'Studio / R&Bae events are not supported in v1'
        : 'This Hakkasan row cannot be imported as a bookable event'
    );
    err.code = 'HAKKASAN_NOT_IMPORTABLE';
    throw err;
  }
  ...
  if (!location) {
    const err = new Error(`Hakkasan location not found: ${locationId}`);
    err.code = 'HAKKASAN_LOCATION_NOT_FOUND';
    throw err;
  }

  const detailUrl = ensureHakkasanEventCodeOnDetailUrl(
    listingEvent.detailUrl || listingEvent.bookUrl,
    hakkasanEventCode
  );
  if (!detailUrl) {
    const err = new Error('Missing Hakkasan event detail URL');
    err.code = 'HAKKASAN_MISSING_DETAIL_URL';
    throw err;
  }

  const detail = await fetchHakkasanEventDetailInventory(detailUrl);
  if (detail.browserError) {
    const err = new Error(`Could not scrape Hakkasan detail page: ${detail.browserError}`);
    err.code = 'HAKKASAN_DETAIL_SCRAPE_FAILED';
    throw err;
  }
  if (detail.warnings?.length) warnings.push(...detail.warnings);

  if (!Array.isArray(detail.items) || detail.items.length === 0) {
    const err = new Error('No TABLES inventory found on Hakkasan detail page');
    err.code = 'HAKKASAN_EMPTY_INVENTORY';
    throw err;
  }

  const inventoryItems = enrichHakkasanInventorySeatCodes(detail.items);
  ...
  const notes = [
    'Imported from Hakkasan Booketing listing.',
    `hakkasanEventCode: ${hakkasanEventCode}`,
  ...
  console.log(`${LOG_PREFIX} prepareHakkasanEventImport: "${prefill.name}" -> location ${location.name}`);
...
async function undoHakkasanEventImport(hakkasanEventCode) {
  const code = (hakkasanEventCode || '').trim();
  if (!code) {
    const err = new Error('Missing Hakkasan event code');
    err.code = 'HAKKASAN_MISSING_EVENT_CODE';
  ...
  const event = await Event.findOne({ hakkasanEventCode: code });
  if (!event) {
    const err = new Error(`No THE1 event found for hakkasanEventCode ${code}`);
    err.code = 'HAKKASAN_EVENT_NOT_FOUND';
    throw err;
  }
  ...
  if (coeByEventId.has(eventId)) {
    ...
    err.code = 'HAKKASAN_EVENT_IN_COE_USE';
    throw err;
  }
  ...
  return {
    hakkasanEventCode: code,
    deletedEventId: eventId,
    message: 'Hakkasan import undone; event removed from THE1',
  };
}

async function resyncHakkasanEventPricing(hakkasanEventCode) {
  const code = (hakkasanEventCode || '').trim();
  if (!code) {
    const err = new Error('Missing Hakkasan event code');
    err.code = 'HAKKASAN_MISSING_EVENT_CODE';
  ...
  const detailUrl = resolveHakkasanDetailUrl(event.notes, code);
  if (!detailUrl) {
  ...
  return {
    ...
    updatedSeatCount,
    warnings,
    message: 'Hakkasan venue catalog pricing refreshed from Booketing',
  };
}
...
module.exports = {
  ...
  extractHakkasanDetailUrlFromNotes,
  resolveHakkasanDetailUrl,
  partitionHakkasanEventsByImportStatus,
  isHakkasanImportableRow,
  parseHakkasanDateQueryParam,
