...
const LOG_PREFIX = '[Palm Tree Beach scraper]';
 ...
 * @returns {string}
 */
function cleanPalmTreeBeachListingName(name, href) {
 ...
 * @returns {object}
 */
function normalizePalmTreeBeachEvent(raw, venueMeta) {
  const venueName = venueMeta?.venueName || 'Palm Tree Beach';
  ...
  const name = cleanPalmTreeBeachListingName((raw.text || '').trim(), href) || 'Event';
  ...
  const externalKey = raw.code
    ? `palm-tree-beach|${raw.code}`
    : `palm-tree-beach|${slugify(name)}|${isoDate || dateDisplay}`;
 ...
 */
async function scrapePalmTreeBeachListingPage(page) {
  const warnings = [];
  ...
  const rawRows = [...byCode.values()].filter((row) => {
    if (!inferPalmTreeBeachFromEventCode(row.code)) return false;
  ...
  if (rawRows.length === 0 && byCode.size > 0) {
    warnings.push(
      `No Palm Tree Beach events in ${currentYm} or ${nextYm} after month filter (${byCode.size} total links on page)`
    );
  }
...
/**
 * Fetch Palm Tree Beach events preview from Booketing calendar.
 * @returns {Promise<object>}
 */
async function fetchPalmTreeBeachEventsPreview() {
  const cfg = getPalmTreeBeachVenueConfig();
  const warnings = [];
  ...
  try {
    const { result, browserError: pageError } = await withBrowserPage(
      cfg.listingUrl,
      async (page) => scrapePalmTreeBeachListingPage(page),
    ...
    if (pageError) {
      ...
      const events = (result?.rawRows || [])
        .map((raw) =>
          normalizePalmTreeBeachEvent(raw, {
            venueName: cfg.venueName,
            category: cfg.category,
          })
        )
        .filter((e) => e.eventCode && inferPalmTreeBeachFromEventCode(e.eventCode))
        .sort((a, b) => {
          const da = a.isoDate || '';
          const db = b.isoDate || '';
      ...
      console.log(
        `${LOG_PREFIX} fetchPalmTreeBeachEventsPreview: ${events.length} event(s)${browserError ? ` (browser error: ${browserError})` : ''}`
      );
    ...
    console.error(`${LOG_PREFIX} fetchPalmTreeBeachEventsPreview error:`, browserError);
  }
 ...
 * @returns {string|null}
 */
function buildPalmTreeBeachEventDetailUrl(detailUrl, eventCode) {
...
/**
 * @param {string} detailUrl
 * @param {string} palmTreeBeachEventCode
 * @returns {string|null}
 */
function ensurePalmTreeBeachEventCodeOnDetailUrl(detailUrl, palmTreeBeachEventCode) {
  return buildPalmTreeBeachEventDetailUrl(detailUrl, palmTreeBeachEventCode);
}

module.exports = {
  fetchPalmTreeBeachEventsPreview,
  normalizePalmTreeBeachEvent,
