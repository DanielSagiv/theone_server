...
const LOG_PREFIX = '[TAO Beach scraper]';
...
/**
 * Whether href is a generic placeholder event path (no slug).
 * @param {string} href
 * @returns {boolean}
 */
function isGenericBookHref(href) {
  return /\/event\/\d+\/\d+\/?(?:\?|$)/i.test(String(href || ''));
}

/**
 * v1: skip generic "Book" placeholder calendar rows (no named flyer).
 * @param {string} name
 * @param {object} raw
 * @returns {boolean}
 */
function isTaoBeachGenericBookRow(name, raw = {}) {
  const n = String(name || '').replace(/\s+/g, ' ').trim();
  if (!n || /^book$/i.test(n)) return true;
  if (raw.hasNameEl === false) return true;
  if (isGenericBookHref(raw.href) && !raw.hasNameEl) return true;
  return false;
}
 ...
 * @returns {string}
 */
function cleanTaoBeachListingName(name, href) {
  ...
  n = n.replace(/^Further transforming the Las Vegas pool scene.*$/i, '').trim();
 ...
 * @returns {object}
 */
function normalizeTaoBeachEvent(raw, venueMeta) {
  const venueName = venueMeta?.venueName || 'TAO Beach';
  ...
  const name = cleanTaoBeachListingName((raw.text || '').trim(), href) || 'Event';
  ...
  const isGenericBookRow = isTaoBeachGenericBookRow(name, raw);
  const externalKey = raw.code
    ? `tao-beach|${raw.code}`
    : `tao-beach|${slugify(name)}|${isoDate || dateDisplay}`;
  ...
  return {
    ...
    skipReason: isGenericBookRow ? 'Generic Book placeholder days are not supported in v1' : null,
  };
}
 ...
 */
async function scrapeTaoBeachListingPage(page, listingUrl) {
  const warnings = [];
...
/**
 * Fetch TAO Beach events preview from Booketing calendar.
 * @returns {Promise<object>}
 */
async function fetchTaoBeachEventsPreview() {
  const cfg = getTaoBeachVenueConfig();
  const warnings = [];
  ...
  try {
    const { result, browserError: pageError } = await withBrowserPage(
      cfg.listingUrl,
      async (page) => scrapeTaoBeachListingPage(page, cfg.listingUrl),
    ...
    if (pageError) {
      ...
      console.log(
        `${LOG_PREFIX} fetchTaoBeachEventsPreview: ${events.length} event(s)${browserError ? ` (browser error: ${browserError})` : ''}`
      );
    ...
    console.error(`${LOG_PREFIX} fetchTaoBeachEventsPreview error:`, browserError);
  }
...
module.exports = {
  fetchTaoBeachEventsPreview,
  normalizeTaoBeachEvent,
